...
Open **`https://<HOST_IP>:5601/`** (self-signed cert — browser warns once, choose Advanced → Continue) → log in as `admin` / your `OPENSEARCH_ADMIN_PASSWORD`.
...
- **Service names** (for `docker compose` commands) are `opensearch`, `tlsoc-dashboards`,
  `kafka`, `logstash` — list them with `docker compose config --services`. The *container*
  names are `tlsoc-opensearch`, `tlsoc-dashboards`, `tlsoc-kafka`, `tlsoc-logstash`.
- **Something failed to start?** Look at its logs first:
  `docker logs --tail 50 tlsoc-opensearch` (or `tlsoc-dashboards`, …).
- **`tlsoc-opensearch` exits immediately with `Password … failed validation`** — your
  `OPENSEARCH_ADMIN_PASSWORD` is too weak or contains `admin` (OpenSearch 2.12+ rejects
  passwords similar to the username). Fix it in `.env` and re-run `./install.sh`
  (the installer pre-checks this, so you'd normally be stopped before boot).
- **`Conflict. The container name … is already in use`** — a container from a previous
  install survived a deleted project folder. `./install.sh` now removes stale `tlsoc-*`
  containers automatically; manually: `docker rm -f <name>` and re-run.
- **Wrong `HOST_IP` detected (VPN address like `10.8.…` / `100.x`)** — the installer skips
  VPN/virtual interfaces (`tun*`, `tailscale*`, `wg*`, …), but if it still picks wrong, set
  `HOST_IP=<your-LAN-IP>` in `.env` and re-run — endpoints must be able to reach that IP.
- **Browser says the connection is not private** — expected on first use: the dashboards
  serve HTTPS with a self-signed certificate generated by `install.sh`. Choose
  Advanced → Continue, or install a real certificate (below).
- **`curl https://localhost:9200` says Unauthorized** — that's OpenSearch security working;
  pass credentials: `curl -sk -u admin:'<password>' https://localhost:9200`.
...
1. **Replace the demo certificates** with real ones (node + admin certs), set
   ...
   Also replace the self-signed dashboards certificate (`certs/dashboards/`) with a real one.
